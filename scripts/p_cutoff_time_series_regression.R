# Lin Poyraz 
# Usage: p_cutoff_time_series_regression.R <expression directory> <output directory> <sample name file>
...
subset = read.table(args[3], header = F)
print(head(subset))
...
results = data.frame("chr" = genes, "start" = rep(1000, length(genes)), "end" = rep(1000, length(genes)), 
	"Gene" = genes, "Ensembl" = genes, "P.value" = rep(1000, length(genes)), "FDR" = rep(1000, length(genes)), "Bonferroni" = rep(1000, length(genes)), "GC" = rep(1000, length(genes)), "Beta" = rep(1000, length(genes)), "SD" = rep(1000, length(genes)))
...
# save results 
output = args[2]
f = paste(output, "/", args[3], "time_series_regression_results.txt", sep = "") 
fwrite(results[,which(!names(results) %in% c("SD"))], f, quote = F, row.names = F, col.names = T, sep = "\t", na = NA)
print(head(results))
