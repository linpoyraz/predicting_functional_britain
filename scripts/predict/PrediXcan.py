...
# modified by Lin Poyraz to remove function to make complements for indels. Jul 2022
...
class TranscriptionMatrix:
    ...
    def update(self, gene, weight, ref_allele, allele, dosage_row):
        ...
        if gene in self.gene_index: #assumes strands are aligned to PrediXcan reference and dosage coding 0 to 2
            if ref_allele == allele: #BUG: or self.getComplements(ref_allele) == allele: # assumes non-ambiguous SNPs to resolve strand issues:
    ...
    def save(self, pred_exp_file):
        ...
        with gzip.open(pred_exp_file, 'w') as outfile:
...
def main():
    ...
    PRED_EXP_FILE = args.pred_exp if args.pred_exp else os.path.join(OUTPUT_DIR, "predicted_expression.txt.gz")
